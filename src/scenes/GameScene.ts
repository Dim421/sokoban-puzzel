...
import {
  ...
  TweenMoveTypes,
  SokobanMode,
} from "~enums/SokobanEnums";
import {
  GetBoxTargetColor,
  GetBoxAnchorColor,
  GetBrowserMobileMode,
  GetSokobanTileMixinMap,
} from "~util/SokobanBoxUtil";
...
export default class GameScene extends Phaser.Scene {
  ...
  create(data: { level: number }) {
    ...
    this.player = this.layer
      .createFromTiles(playerIndexId, playerReplaceId, {
        key: "tiles",
        frame: 52,
      })
      .pop();
    ...
    this.input.keyboard.on("keydown-N", () => {
      this.scene.restart({
        level: Math.max(this.currentLevel - 1, SokobanLevel.FirstLevel),
      });
    });

    this.input.keyboard.on("keydown-M", () => {
      this.scene.restart({
        level: Math.min(this.currentLevel + 1, SokobanLevel.HighestLevel),
      });
    });
    ...
    this.stepsLabel = this.add.text(
      540,
      10,
      `Level: ${this.currentLevel}  Steps: ${this.steps}`,
      {
        fontSize: "20px",
      }
    );
    ...
    const gameWidth =
      typeof this.game.config.width === "number" ? this.game.config.width : 0;
    const gameHeight =
      typeof this.game.config.height === "number" ? this.game.config.height : 0;
    const parent = new Phaser.Structs.Size(
      this.scale.gameSize.width,
      this.scale.gameSize.height
    );
    const sizer = new Phaser.Structs.Size(
      gameWidth,
      gameHeight,
    ...
    if (this.MOBILEMODE) {
      ...
      // up-stick
      this.joySticks!["up"] = this.add
        .rectangle(65, screenHeight - 185, 50, 80, 0x5090c7, 0.5)
        .setInteractive();
      // down-stick
      this.joySticks!["down"] = this.add
        .rectangle(65, screenHeight - 55, 50, 80, 0x5090c7, 0.5)
        .setInteractive();
      // left-stick
      this.joySticks!["left"] = this.add
        .rectangle(0, screenHeight - 120, 80, 50, 0x5090c7, 0.5)
        .setInteractive();
      // right-stick
      this.joySticks!["right"] = this.add
        .rectangle(130, screenHeight - 120, 80, 50, 0x5090c7, 0.5)
        .setInteractive();
      // rest-stick
      this.joySticks!["reset"] = this.add
        .rectangle(screenWidth - 50, screenHeight - 140, 80, 50, 0x5090c7, 0.5)
        .setInteractive();
      // backToTitle-stick
      this.joySticks!["main"] = this.add
        .rectangle(screenWidth - 50, screenHeight - 60, 80, 50, 0xdb6056, 0.5)
        .setInteractive();
  ...
  private tweenMoved(
    ...
    if (tweenType === TweenMoveTypes.OnlyBox && !movePosition) {
      throw new Error(
        "The TweenMoveTypes.OnlyBox need 'movePosition' parameter' value. "
      );
    }

    let boxData:
      | { box: Phaser.GameObjects.Sprite; color: BoxColors }
      | undefined;
    ...
    const nextSlideBlock = this.hasDestTileAtPos(pos.x, pos.y, [
      SpecialMaterials.SlideBlock,
    ]);
    const nextHoleBlock = this.hasDestTileAtPos(pos.x, pos.y, [
      SpecialMaterials.HoleBlock,
    ]);
    ...
    if (boxData) {
      ...
      const siblingBox = this.getBoxDataAtPos(
        pos.x + pos.offsetX,
        pos.y + pos.offsetY
      );
      ...
      if (
        siblingBox ||
        this.hasDestTileAtPos(box.x + pos.offsetX, box.y + pos.offsetY, [
          Materials.Wall,
        ])
      ) {
        return;
      ...
      const boxNextSlideBlock = this.hasDestTileAtPos(
        box.x + pos.offsetX,
        box.y + pos.offsetY,
        [SpecialMaterials.SlideBlock]
      );
      const boxNextHoleBlock = this.hasDestTileAtPos(
        box.x + pos.offsetX,
        box.y + pos.offsetY,
        [SpecialMaterials.HoleBlock]
      );
        ...
        Object.assign(
          {
            ...
            onComplete: () => {
              ...
              if (target && anchor) {
                ...
                const coverTarget = this.hasDestTileAtPos(box.x, box.y, [
                  target,
                ]);
              ...
              if (boxNextSlideBlock) {
                this.tweenMoved(orientation, TweenMoveTypes.OnlyBox, {
                  x: box.x - pos.offsetX,
                  y: box.y - pos.offsetY,
                });
              }
  ...
  private makeColorBoxes(levelInfo: SokobanLevelInfo): void {
    ...
    this.BOXCOLORS.forEach((i) => {
      ...
      // init box tile
      this.boxes![i] = this.layer!.createFromTiles(index_id, replace_id, {
        key: "tiles",
        frame: frame_id,
      ...
      if (levelInfo.mixin) {
        ...
        if (targets) {
          for (let target of targets) {
            ...
            let mixinBoxes = this.layer!.createFromTiles(index_id, replace_id, {
              key: "tiles",
              frame: frame_id,
            }).map((box) => box.setOrigin(0));
  ...
  private changeBoxColor(box: Phaser.GameObjects.Sprite, color: BoxColors) {
    if (
      this.gameMode !== SokobanMode.Change ||
      !this.boxes ||
